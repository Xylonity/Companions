...
import dev.xylonity.companions.client.entity.model.TamedIllagerGolemModel;
...
public class TamedIllagerGolemRenderer extends GeoEntityRenderer<TamedIllagerGolemEntity> {

    public TamedIllagerGolemRenderer(EntityRendererProvider.Context renderManager, int totalFrames, int ticksPerFrame) {
        super(renderManager, new TamedIllagerGolemModel());
    ...
    public TamedIllagerGolemRenderer(EntityRendererProvider.Context renderManager) {
        this(renderManager, 8, TamedIllagerGolemEntity.ELECTRICAL_CHARGE_DURATION / 8);
    }

    @Override
    public @NotNull ResourceLocation getTextureLocation(@NotNull TamedIllagerGolemEntity animatable) {
        return new ResourceLocation(CompanionsCommon.MOD_ID, "textures/entity/tamed_illager_golem.png");
    }

    private static class ElectricConnectionLayer extends GeoRenderLayer<TamedIllagerGolemEntity> {
        private final ResourceLocation texture;
        private final int totalFrames;
        private final int ticksPerFrame;

        public ElectricConnectionLayer(GeoRenderer<TamedIllagerGolemEntity> renderer, ResourceLocation texture, int totalFrames, int ticksPerFrame) {
            super(renderer);
        ...
        @Override
        public void render(PoseStack poseStack, TamedIllagerGolemEntity animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        private int calculateCurrentFrame(TamedIllagerGolemEntity animatable) {
