...
public abstract class ItemRendererMixin {
    ...
    @Unique
    private static final Map<Item, ModelResourceLocation> companions$MODELS_2D;

    @ModifyVariable(method = "getModel", at = @At(value = "STORE"), ordinal = 1)
    public BakedModel getModel(BakedModel bakedModel, ItemStack pStack) {
    ...
    private BakedModel replaceBaked(BakedModel bakedModel, ItemStack pStack, ItemDisplayContext pDisplayContext) {
        ...
        if (ctx) {
            ModelResourceLocation loc = companions$MODELS_2D.get(pStack.getItem());
            if (loc != null) {
                return this.itemModelShaper.getModelManager().getModel(loc);
            }
        }
