...
public class CompanionsEntities {
    ...
    private static <X extends Entity> EntityType<X> register(String name, EntityType.EntityFactory<X> entity, MobCategory category, float width, float height, List<Consumer<EntityType.Builder<X>>> properties) {
        return Registry.register(BuiltInRegistries.ENTITY_TYPE, ResourceLocation.fromNamespaceAndPath(Companions.MOD_ID, name), buildEntity(name, entity, category, width, height, properties));
    }

    private static <X extends Entity> EntityType<X> buildEntity(String name, EntityType.EntityFactory<X> entity, MobCategory category, float width, float height, List<Consumer<EntityType.Builder<X>>> properties) {
    ...
    private static Item registerSpawnEggItem(String id, EntityType<? extends Mob> entityType, int primaryEggColour, int secondaryEggColour) {
        Item item = Registry.register(BuiltInRegistries.ITEM, ResourceLocation.fromNamespaceAndPath(Companions.MOD_ID, id), new SpawnEggItem(entityType, primaryEggColour, secondaryEggColour, new Item.Properties()));
