    ...
    public void tick() {
        ...
        if (!this.level().isClientSide) {
            for (LivingEntity entity : level().getEntitiesOfClass(LivingEntity.class, new AABB(getX() - RADIUS, getY() - 1, getZ() - RADIUS, getX() + RADIUS, getY() + 1, getZ() + RADIUS), e -> !Util.areEntitiesLinked(this, e))) {
                if (!entity.hasEffect(CompanionsEffects.holder(CompanionsEffects.FIRE_MARK))) {
                    entity.addEffect(new MobEffectInstance(CompanionsEffects.holder(CompanionsEffects.FIRE_MARK), level().random.nextInt(100, 400), 0, true, true));
