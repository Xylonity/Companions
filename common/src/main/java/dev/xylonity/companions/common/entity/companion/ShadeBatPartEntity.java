...
public class ShadeBatPartEntity extends Mob implements GeoAnimatable {
    ...
    @Override
    public void lerpTo(double pX, double pY, double pZ, float pYRot, float pXRot, int pSteps, boolean pTeleport) {
        final ShadeBatEntity parent = getParent();
        if (parent != null && !parent.isRemoved() && !parent.isBlood()) {
            return;
        }

        super.lerpTo(pX, pY, pZ, pYRot, pXRot, pSteps, pTeleport);
    }
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(PART_INDEX, -1);
    }
