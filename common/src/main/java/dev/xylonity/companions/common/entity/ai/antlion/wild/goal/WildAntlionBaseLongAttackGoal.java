...
public class WildAntlionBaseLongAttackGoal extends AbstractWildAntlionAttackGoal {
    ...
    private void spawnParabolicParticles(LivingEntity target) {
        ...
        if (isHit) {
            ...
            if (antlion.level().random.nextFloat() < 0.35f) TickScheduler.scheduleServer(serverLevel, () -> target.igniteForSeconds(new Random().nextInt(1, 3)), steps / TICK_DELAY_DIVISOR);
        }
