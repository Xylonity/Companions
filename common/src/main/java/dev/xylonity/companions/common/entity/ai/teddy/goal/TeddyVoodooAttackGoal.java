...
public class TeddyVoodooAttackGoal extends AbstractTeddyAttackGoal {
    ...
    public boolean canUse() {
        this.list = teddy.level().getEntitiesOfClass(LivingEntity.class, new AABB(teddy.blockPosition()).inflate(20), e -> e.hasEffect(CompanionsEffects.holder(CompanionsEffects.VOODOO)) && !Util.areEntitiesLinked(e, teddy));
    ...
    protected void performAttack(LivingEntity unused) {
        for (LivingEntity e : list) {
            if (!Util.areEntitiesLinked(e, teddy)) {
                e.removeEffect(CompanionsEffects.holder(CompanionsEffects.VOODOO));
