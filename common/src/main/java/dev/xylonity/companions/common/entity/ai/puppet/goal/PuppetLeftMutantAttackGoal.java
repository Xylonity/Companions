...
public class PuppetLeftMutantAttackGoal extends AbstractPuppetLeftAttackGoal {

    public PuppetLeftMutantAttackGoal(PuppetEntity puppet, int minCd, int maxCd) {
    ...
    protected void performAttack(LivingEntity target) {
        if (target != null) {
            ...
            target.addEffect(new MobEffectInstance(CompanionsEffects.holder(CompanionsEffects.VOODOO), puppet.getRandom().nextInt(100, 300), 0, true, true, true));
        }
    ...
    protected boolean hasRequiredArm() {
        return puppet.inventory.getItem(1).is(CompanionsItems.MUTANT_ARM.get());
    }
