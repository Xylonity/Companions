...
public class TeddyAttackGoal extends AbstractTeddyAttackGoal {
    ...
    public boolean canUse() {
        return super.canUse() && teddy.getTarget() != null && teddy.distanceTo(teddy.getTarget()) <= 3 && teddy.getPhase() == 1;
    }
    ...
    protected void performAttack(LivingEntity target) {
        if (teddy.hasLineOfSight(target)) {
            ...
            if (new Random().nextFloat() < 0.35f) {
                target.addEffect(new MobEffectInstance(CompanionsEffects.holder(CompanionsEffects.VOODOO), new Random().nextInt(100, 300), 0, true, true, true));
            }
        }
