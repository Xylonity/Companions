...
public class BlackHoleRenderer extends GeoEntityRenderer<BlackHoleProjectile> {
    ...
    public @NotNull ResourceLocation getTextureLocation(@NotNull BlackHoleProjectile animatable) {
        if (animatable.getTickCount() == 18 || animatable.getTickCount() == 19) {
            return Companions.of("textures/entity/black_hole_white.png");
        }
        if (animatable.getTickCount() == 20 || animatable.getTickCount() == 21) {
            return Companions.of("textures/entity/black_hole_black.png");
        }
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, BlackHoleProjectile animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
