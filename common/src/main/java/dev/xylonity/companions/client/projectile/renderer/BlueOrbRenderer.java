...
public class BlueOrbRenderer extends GeoEntityRenderer<BlueOrbProjectile> {
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, BlueOrbProjectile animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
        if (isReRender) {
    ...
    @Override
    protected void applyRotations(BlueOrbProjectile animatable, PoseStack poseStack, float ageInTicks, float rotationYaw, float partialTick, float nativeScale) {
