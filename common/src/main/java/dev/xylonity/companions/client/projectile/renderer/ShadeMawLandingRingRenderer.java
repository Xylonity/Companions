        ...
        for (int sector = 0; sector < 8; sector++) {
            ...
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < cols; j++) {
                    ...
                    buffer.addVertex(matrix, xs[i][j], 0f, zs[i][j]).setColor(COLOR_B_R, COLOR_B_G, COLOR_B_B, alphas[i][j]);
                    buffer.addVertex(matrix, xs[i][j + 1], 0f, zs[i][j + 1]).setColor(COLOR_B_R, COLOR_B_G, COLOR_B_B, alphas[i][j + 1]);
                    buffer.addVertex(matrix, xs[i + 1][j + 1], 0f, zs[i + 1][j + 1]).setColor(COLOR_B_R, COLOR_B_G, COLOR_B_B, alphas[i + 1][j + 1]);
                    buffer.addVertex(matrix, xs[i + 1][j], 0f, zs[i + 1][j]).setColor(COLOR_B_R, COLOR_B_G, COLOR_B_B, alphas[i + 1][j]);
                }
    ...
    private static void annulus(VertexConsumer buffer, Matrix4f matrix, float[] radii, float[] alphas, float red, float green, float blue) {
        for (int i = 0; i < radii.length - 1; i++) {
            ...
            for (int j = 0; j < SEGMENTS; j++) {
                ...
                buffer.addVertex(matrix, cos0 * r0, 0f, sin0 * r0).setColor(red, green, blue, a0);
                buffer.addVertex(matrix, cos1 * r0, 0f, sin1 * r0).setColor(red, green, blue, a0);
                buffer.addVertex(matrix, cos1 * r1, 0f, sin1 * r1).setColor(red, green, blue, a1);
                buffer.addVertex(matrix, cos0 * r1, 0f, sin0 * r1).setColor(red, green, blue, a1);
            }
