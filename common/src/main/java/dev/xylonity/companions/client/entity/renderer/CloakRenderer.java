...
public class CloakRenderer extends GeoEntityRenderer<CloakEntity> {
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, CloakEntity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
        if (animatable instanceof IPhantomEffectEntity phantomEntity && phantomEntity.isPhantomEffectActive()) {
            Player player = Minecraft.getInstance().player;
            ...
            if (player != null) {
                ...
                if (visibility == PhantomVisibility.TRANSLUCENT) {
                    colour = KnightLibColor.withAlpha(colour, (int) (0.35f * 255));
                }
