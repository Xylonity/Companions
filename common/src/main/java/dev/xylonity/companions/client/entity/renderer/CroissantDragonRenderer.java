...
public class CroissantDragonRenderer extends GeoEntityRenderer<CroissantDragonEntity> {
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, CroissantDragonEntity animatable, BakedGeoModel model, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        final float scale = animatable.getMilkAmount() == 0 ? 0.3f
